...
export class DescargarArchivosController {
  ...
  private ObtenerRutaDeCarpetaPorTipo(type: number) {
    ...
    switch (type) {
      // mascota
      case 1:
        ruta = path.join(__dirname, llaves.carpetaImagenProponenteTrabajo);
        break;
      case 2:
        ruta = path.join(__dirname, llaves.carpetaDocumentoTipoSolicitud);
        break;
      case 3:
        ruta = path.join(__dirname, llaves.carpetaDocumentoSolicitud);
        break;
      case 4:
        ruta = path.join(__dirname, llaves.carpetaDocumentoResultadoEvaluacion);
    }
    return ruta;
  }
